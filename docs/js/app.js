...
function openEditSaveModal(saveId) {
  ...
  const body = `<div class="form-grid">
    ${fieldHtml('اسم الحفظ', 'save_name', s.name)}
  ...
  <p class="muted" style="margin-top:6px">تعديل «الصفحة الحالية» يحذف صفوف الأيام الأحدث ويعيد بناء صف اليوم للمراجعة. «من صفحة» لا يُرسل أي إشعار. لتصفير الحفظ بالكامل استخدم زر «تعديل من الصفر». كل التعديلات تُسجَّل في سجل العمليات.</p>
  <div class="field"><label><input type="checkbox" id="es-notify" style="width:auto;margin-left:6px">إبلاغ الطالب بالتعديل (اسم الحفظ/النهاية) — اختياري، مطفأ افتراضياً</label></div>
  ...
  m.el.querySelector('#md-ok').onclick = async ev => {
    ...
    // اسم الحفظ يُرسل فقط عند تغييره فعلياً
    if (String(f.save_name ?? '').trim() !== String(s.name ?? '')) fields.save_name = f.save_name;
